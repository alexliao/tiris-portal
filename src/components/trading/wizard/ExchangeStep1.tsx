...
export const ExchangeStep1: React.FC<ExchangeStep1Props> = ({
  exchanges,
  selectedExchange,
  setSelectedExchange,
  iconServiceBaseUrl,
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold text-gray-900 mb-2">
        {t('exchanges.wizard.step1.title')}
      </h2>
      <p className="text-gray-600 mb-6">
        {t('exchanges.wizard.step1.description')}
      </p>
              ...
              <div className="flex-1 min-w-0">
                ...
                {exchange.sandbox && (
                  <span className="inline-flex items-center px-2 py-1 mt-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">
                    {t('exchanges.wizard.step1.sandboxBadge')}
                  </span>
                )}
              </div>
            </div>
      ...
      {exchanges.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          <p>{t('exchanges.wizard.step1.noExchanges')}</p>
        </div>
      )}
    </div>
  );
